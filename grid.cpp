 ...
 * Grid::~Grid()
 *
 * Destruct a grid if necessary. The current implementation doesn't require this function.
 * If the implementation chenges, then this destructor needs to be implemented accordingly.
 *
 */
Grid::~Grid() {
}
...
Grid::Grid(int width, int height) :
		theGrid(width * height, DEAD), num_columns(width), num_rows(height) {
}
...
void Grid::resize(int width, int height) {
	//If the new height is smaller than the current height then remove the last rows
	//of cells starting from num_columns * height until the end from the vector.
	if (height < num_rows) {
		theGrid.erase(theGrid.begin() + num_columns * height, theGrid.end());
		num_rows = height;
	}
	//If the new width is smaller than the current width then remove columns
	//of cells.
	//This operation starts from the end of the vector and sets 2 indexes
	//between cells that need to be deleted: one at the index of the array where the first cell is deleted
	//and second at the cell where is stops deleting.
	//As this is the vector the remaining cells on the right-hand side will get transfered to the left.
	//The indexes are set row by row depending how many cells need to be deleted.
	if (width < num_columns) {
		int columns_to_delete = num_columns - width;
		for (int i = 0; i < num_rows; i++) {
			theGrid.erase(theGrid.end() - width * i - columns_to_delete,
					theGrid.end() - width * i);
		}
		num_columns = width;
	}
	//A vector of dead cells is computed and added at the end of the current grid's vector.
	if (height > num_rows) {
		std::vector<Cell> dead_cells_vector((height - num_rows) * num_columns,
				DEAD);
		theGrid.insert(theGrid.end(), dead_cells_vector.begin(),
				dead_cells_vector.end());
		num_rows = height;
	}
	//A similar operationn as deleting columns of cells but instead this instruction adds dead cells.
	if (width > num_columns) {
		std::vector<Cell> dead_cells_vector(width - num_columns, DEAD);
		for (int i = 0; i < num_rows; i++) {
			theGrid.insert(theGrid.end() - width * i, dead_cells_vector.begin(),
					dead_cells_vector.end());
		}
		num_columns = width;
	}
...
int Grid::get_index(int x, int y) const {
	if (x >= num_columns || x < 0 || y >= num_rows || y < 0) {
		throw std::runtime_error(
				"Coordinates are out of the grid size, not valid");
	}
 ...
 *      std::exception or sub-class if x,y is not a valid coordinate within the grid.
 */
void Grid::set(int x, int y, Cell cell) {
	if (x >= num_columns || x < 0 || y >= num_rows || y < 0) {
		throw std::runtime_error(
				"Coordinates are out of the grid size, not valid");
	}
...
Cell& Grid::operator()(int x, int y) {
	return theGrid.at(get_index(x, y));
}
...
Grid Grid::crop(int x0, int y0, int x1, int y1) const {
	...
	//Testing the coordinates to not be out of bounds.
	//If they are an error will be thrown.
	get_index(x0, y0);
	get_index(x1 - 1, y1 - 1);

	//Contruct a vector that takes only the cells inside the grid between the coordinates
	//specified.
	std::vector<Cell> temp;
	for (int i = y0; i < y1; i++) {
		temp.insert(temp.end(), theGrid.begin() + x0 + num_columns * i,
				theGrid.begin() + num_columns * i + x1);
	}
	//Initializa a new grid that will take the size and grid required.
	Grid subgrid(x1 - x0, y1 - y0);
	subgrid.theGrid = temp;
	return subgrid;
}
...
void Grid::merge(Grid other, int x0, int y0, bool alive_only) {
	//Directly set the cell at every position from the other grid in the current grid
	//according to hte alive_only parameter.
	if (!alive_only == true) {
		for (int i = 0; i < other.num_rows; i++) {
			for (int j = 0; j < other.num_columns; j++) {
				set(j + x0, i + y0, other(j, i));
			}
		}
	} else {
		for (int i = 0; i < other.num_rows; i++) {
			for (int j = 0; j < other.num_columns; j++) {
				if (get(j + x0, i + y0) == Cell::DEAD) {
					set(j + x0, i + y0, other(j, i));
				}
			}
		}
...
Grid Grid::rotate(int rotation) const {
	//Calculate if the grid needs to be rotated 1,2 or 3 times and in which direction
	//, for greater than 3,-3 degrees.
	int times = rotation % 4;
	//Depending how many times needs to be rotated and the direction set the cells
	//in a temporary grid according to the rotated current grid.
	if (times == 1 || times == -3) {
		Grid temp(num_rows, num_columns);
		for (int i = 0; i < num_columns; i++) {
			for (int j = 0; j < num_rows; j++) {
				temp.set(num_rows - 1 - j, i, get(i, j));
			}
		}
		return temp;
	} else if (times == 2 || times == -2) {
		Grid temp(num_columns, num_rows);
		for (int i = 0; i < num_columns; i++) {
			for (int j = 0; j < num_rows; j++) {
				temp.set(num_columns - 1 - i, num_rows - j - 1, get(i, j));
			}
		}
		return temp;
		...
		for (int i = 0; i < num_columns; i++) {
			for (int j = 0; j < num_rows; j++) {
				temp.set(j, num_columns - 1 - i, get(i, j));
			}
		}
		return temp;
	}
	//If the grid hasn't been modified then return a copy of it.
	Grid temp(num_columns, num_rows);
	temp.theGrid = theGrid;
	return temp;
}
